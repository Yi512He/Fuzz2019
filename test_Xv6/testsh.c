...
writefile(char *name, char *data)
{
  ...
  if(fd < 0){
    printf(1, "testsh: could not write %s\n", name);
    exit();
  }
  if(write(fd, data, strlen(data)) != strlen(data)){
    printf(1, "testsh: write failed\n");
    exit();
  }
  close(fd);
}
...
one(char *cmd, char *expect, int tight)
{
  ...
  if(pid == 0){
    close(0);
    if(open(infile, 0) != 0){
      printf(1, "testsh: child open != 0\n");
      exit();
    }
    close(1);
    if(open(outfile, O_CREATE|O_WRONLY) != 1){
      printf(1, "testsh: child open != 1\n");
      exit();
    }
    char *argv[2];
    argv[0] = shname;
    argv[1] = 0;
    exec(shname, argv);
    printf(1, "testsh: exec %s failed\n", shname);
    exit();
  }

  if(wait() != pid){
    printf(1, "testsh: unexpected wait() return\n");
    exit();
  }
  unlink(infile);
  ...
  if(strstr(out, expect) != 0){
    if(tight && strlen(out) > strlen(expect) + 10){
      printf(1, "testsh: saw expected output, but too much else as well\n");
      return 0; // fail
    }
    return 1; // pass
  }
